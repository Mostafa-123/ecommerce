            ...
            <div class="row">
                            ...
                            <div class="swiper-container">
                                <div class="swiper-wrapper">
                                    ...
                                    @foreach (explode(',', $product->images) as $gimage)
                                        <div class="swiper-slide product-single__image-item">
                                            <img loading="lazy" class="h-auto" src="{{asset('uploads/products')}}/{{$gimage}}"
                                                width="674" height="674" alt="{{$product->name}}" />
                                            <a data-fancybox="gallery" href="{{asset('uploads/products')}}/{{$gimage}}"
                                                data-bs-toggle="tooltip" data-bs-placement="left" title="Zoom">
                                                <svg width="16" height="16" viewBox="0 0 16 16" fill="none"
                                                    xmlns="http://www.w3.org/2000/svg">
                                                    <use href="#icon_zoom" />
                            ...
                            <div class="swiper-container">
                                <div class="swiper-wrapper">
                                    ...
                                    @foreach (explode(',', $product->images) as $gimage)
                                        ...
                                        <div class="swiper-slide product-single__image-item"><img loading="lazy" class="h-auto"
                                                src="{{asset('uploads/products')}}/{{$gimage}}" width="104" height="104"
                                                alt="{{$product->name}}" /></div>
                                    @endforeach
                                </div>
                            </div>
                ...
                <div class="col-lg-5">
                    ...
                    <div class="product-single__addtolinks">
                        <form method="post" action="{{Cart::instance('wishlist')->content()->where('id', $product->id)->count() > 0 ?
                                         route('wishlist.item.remove',['rowId'=>Cart::instance('wishlist')->content()->where('id', $product->id)->first()->rowId]) : route('wishlist.add')}}" id="wishlist-form">
                            @csrf
                            ...
                            <a href="javascript:void(0)"
                                class="menu-link menu-link_us-s add-to-wishlist
                                                {{Cart::instance('wishlist')->content()->where('id', $product->id)->count() > 0 ? "filled-heart" : "" }}"
                                onclick="document.getElementById('wishlist-form').submit();"><svg width="16" height="16" viewBox="0 0 20 20"
                                    fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <use href="#icon_heart" />
                                </svg>
                                <span>{{Cart::instance('wishlist')->content()->where('id', $product->id)->count() > 0 ? "Add To Wishlist" : "Remove from Wishlist"}}</span>
                            </a>
                        </form>
        ...
        <section class="products-carousel container">
            ...
            <div id="related_products" class="position-relative">
                <div class="swiper-container js-swiper-slider" data-settings='{
                    ...
                    <div class="swiper-wrapper">
                        @foreach ($releated_products as $rproduct)
                            <div class="swiper-slide product-card">
                                <div class="pc__img-wrapper">
                                    <a href="{{route('shop.product.details', ['product_slug' => $rproduct->slug])}}">
                                        ...
                                        @foreach (explode(',', $rproduct->images) as $rgimage)

                                            <img loading="lazy" src="{{asset('uploads/products')}}/{{$rgimage}}" width="330"
                                ...
                                <div class="pc__info position-relative">
                                    ...
                                    <form method="post" action="{{Cart::instance('wishlist')->content()->where('id', $rproduct->id)->count() > 0 ?
                                         route('wishlist.item.remove',['rowId'=>Cart::instance('wishlist')->content()->where('id', $rproduct->id)->first()->rowId]) : route('wishlist.add')}}">
                                        @csrf
                                        ...
                                        <input type="hidden" name="id" value="{{$rproduct->id}}" />
                                        <input type="hidden" name="name" value="{{$rproduct->name}}" />
                                        <input type="hidden" name="price" value="{{$rproduct->price}}" />
                                        <input type="hidden" name="quantity" value="1" />

                                        <button type="submit"
                                        class="pc__btn-wl position-absolute top-0 end-0 bg-transparent border-0 js-add-wishlist {{Cart::instance('wishlist')->content()->where('id', $product->id)->count() > 0 ? "filled-heart" : "" }}"
                                            title={{Cart::instance('wishlist')->content()->where('id', $rproduct->id)->count() > 0 ? "Add To Wishlist" : "Remove from Wishlist"}}>
